...
public class TransactionManager {
	...
	public static HashMap<Integer, Transaction> beginTransaction(String processedLine, int timestamp, int transactionId) {
		...
		transactionTableHashMap.put(transactionId, newTransaction);
		System.out.print(processedLine + "	" + "Begin T" + newTransaction.getTransactionId()
				+ ": Record is added to transaction table with Tid=" + newTransaction.getTransactionId() + " and TS(T"
	...
	public static HashMap<Integer, Transaction> requestLock(String processedLine, String dataItem, int transactionId,
			String operation) {

		operation = operation.equals("r") ? "Read" : "Write";
		Transaction t = transactionTableHashMap.get(transactionId);

		if (t.getTransactionState().equals("Active")) {
			transactionTableHashMap = RigorousTwoPhaseLocking.activeTransaction(processedLine, dataItem, t, operation);
		} else if (t.getTransactionState().equals("Block")) {
			transactionTableHashMap = RigorousTwoPhaseLocking.blockTransaction(processedLine, dataItem, t, operation);
		} else if (t.getTransactionState().equals("Abort")) {
			System.out.print("Transaction T" + t.getTransactionId() + "is aborted.");
		} else if (t.getTransactionState().equals("Commit")) {
			System.out.print("Transaction T" + t.getTransactionId() + "is committed.");
		}
		return transactionTableHashMap;
	}
	...
	public static void abortTransaction(String processedLine, Transaction t) {
		...
		transactionTableHashMap.put(t.transactionId, t);
