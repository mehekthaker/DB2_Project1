...
public class RigorousTwoPhaseLocking {
	...
	public static HashMap<Integer, Transaction> blockTransaction(String processedLine, String dataItem, Transaction t,
		...
		System.out.println(processedLine + "  Transaction T" + t.getTransactionId() + " is in blocked state "
				+ operation + " operation on data item " + dataItem
	...
	public static HashMap<Integer, Transaction> activeTransaction(String processedLine, String dataItem,
			Transaction trans, String operation) {
		...
		if (lockTableHashMap.containsKey(dataItem)) {
			...
			if (record.getLockState().equals("Read") && operation.equals("Read")) {
				...
				System.out.println("Transaction T" + trans.getTransactionId() + " has procured Read Lock on data item "
						+ dataItem + "\n");
				...
				System.out.print("Transaction T" + trans.getTransactionId() + " has procured Write Lock on data item "
						+ dataItem + "\n");
			}
			...
			if (operation.equals("Read")) {
				...
				record.readLockTransactionId.add(trans.getTransactionId());
				System.out.println(processedLine + "  The transaction state for transaction T"
						+ trans.getTransactionId() + " is Active so entry for data item " + dataItem
						+ " has been entered in the lock table and transaction T" + trans.getTransactionId()
						+ " has procured Read Lock on it." + "\n");
			} else if (operation.equals("Write")) {
				record = new Lock(dataItem, operation, trans.getTransactionId());
				System.out.println(processedLine + "  The transaction state for transaction T"
						+ trans.getTransactionId() + " is Active so entry for data item " + dataItem
						+ " has been entered in the lock table and transaction T" + trans.getTransactionId()
						+ " has procured Write Lock on it." + "\n");
			}
	...
	public static void releaseLock(String processedLine, Transaction trans, String dataItem) {
		...
		if (lockRecord.getLockState().equals("Write") || lockRecord.readLockTransactionId.size() == 1) {
			...
			if (lockRecord.readLockTransactionId.size() == 1) {
				...
				System.out.println(
						"Transaction T" + trans.getTransactionId() + " has released write lock on " + dataItem);
			}
	...
	public static Transaction procureLock(String processedLine, Transaction trans, String dataItem, Lock lockRecord) {
		...
		if (!wo.isEmpty()) {
			...
			System.out.println("Running its blocked operations...");
		}
		while (!wo.isEmpty()) {
